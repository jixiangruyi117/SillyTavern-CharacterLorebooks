...
test('cancels a stale large build after it yields to a newer rebuild', async () => {
    let cancelled = false;
    let yields = 0;
    const result = await buildLorebookIndexAsync({
        worldNames: ['Book 0', 'Book 1', 'Book 2'],
        characters: [
            { name: 'Role 0', avatar: 'role-0.png', data: { extensions: { world: 'Book 0' } } },
            { name: 'Role 1', avatar: 'role-1.png', data: { extensions: { world: 'Book 1' } } },
            { name: 'Role 2', avatar: 'role-2.png', data: { extensions: { world: 'Book 2' } } },
        ],
    }, {
        chunkSize: 1,
        yieldToMain: async () => {
